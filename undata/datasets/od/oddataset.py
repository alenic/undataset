...
from undata.datasets.od.voc_reader import VOCReader
from undata.datasets.od.voc_writer import VOCWriter
from undata.datasets.od.yolo_reader import YOLOReader
from undata.datasets.od.yolo_writer import YOLOWriter
from undata.untypes import BBoxFormatType

from PIL import Image


class ODDataset(UNDataset):
    """
    Object Detection Dataset.

    Attributes:
        rootdir (str): The root path of the dataset.
        labels_map (Dict[int, str]): The {label_id: "label_name"} global map
    """
    ...
    def filter_image_size(
        ...
        """
        Returns a new ODDataset containing only samples where
    ...
    def filter_bbox_size(
        ...
        """
        Returns a new ODDataset where each sample contains only bounding boxes
    ...
    def draw_sample(
        ...
        return ODPlotter(cmap).draw_sample(
            self.rootdir,
    ...
    # --------- JSON
    @classmethod
    def read_json(cls, json_file: str) -> "ODDataset":
        with open(json_file, "r") as fp:
            json_str = fp.read()

        undataset = ODDataset().model_validate_json(json_str)
    ...
    # --------- Pandas Dataframe
    @classmethod
    def read_dataframe(cls, df, rootdir: str = ".") -> "ODDataset":
        undataset = ODDataset(rootdir=rootdir)
        ...
        for idx, group in tqdm(
            ...
            undataset.sample[idx] = ODSample.from_dataframe(group)
    ...
    def to_dataframe(self) -> pd.DataFrame:
        df = pd.DataFrame()
        frames = []
        for idx in tqdm(self.sample.keys(), desc="Convert ODDataset as DataFrame"):
