...
class ODSample(UNSample):
    ...
    @staticmethod
    def from_dataframe(df: pd.DataFrame) -> "ODSample":
        if df.empty:
            raise ValueError("df is empty")

        sample = ODSample(image_path=str(df.iloc[0]["image_path"]))
        sample.image_w = (
        ...
        tags = df.iloc[0].get("tags", None)
        sample.tags = None if (isinstance(tags, float) and pd.isna(tags)) else tags
